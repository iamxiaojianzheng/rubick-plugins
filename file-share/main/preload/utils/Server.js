...
function getClientIp(req) {
  ...
  console.log(
    "sourceip %s, realip %s, clientip %s",
    sourceip,
    realip,
    clientip,
  );
...
const initApp = () => {
  ...
  app.get("/api/files", function (req, res) {
    ...
    return res.json({
      code: 200,
      data: { path: filePaths, files: FileUtil.listFiles(sourceFilePath) },
    });
  });
  // download
  app.get("/api/download", function (req, res) {
    ...
    if (fs.lstatSync(sourceFilePath).isDirectory()) {
      ...
      let destZipFile =
        path.join(FileUtil.getTempFilePath(), fileName + "_" + timestamp) +
        ".zip";
      if (fs.existsSync(destZipFile)) {
        doDownload(destZipFile, () => FileUtil.clearTempFileDelay(destZipFile));
      } else {
        ZipUtil.zipDirectory(sourceFilePath, destZipFile)
          .then((event) => {
            doDownload(destZipFile, () =>
              FileUtil.clearTempFileDelay(destZipFile),
            );
          })
          .catch((error) => {
            console.log(error);
