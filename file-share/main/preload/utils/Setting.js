...
function getUploadPath() {
  return AppDatabase.getStorageItem(
    uploadPathKey + ":" + getMachineId(),
...
function updateChunkSize(chunkSize) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!isNumber(chunkSize)) {
      return reject({
        success: false,
        message: "更新分片大小失败，值不是数字",
      });
    }
    if (chunkSize <= 0) {
      return reject({
        success: false,
        message: "更新分片大小失败，值应该大于0",
      });
    }
    // 值没变，不更新
...
function updateSetting(setting) {
  ...
  updateUploadPath(setting[uploadPathKey])
    .then((e) => {
      console.log(e);
    })
    .catch((e) => {
  ...
  updateTusEnable(setting[tusEnableKey])
    .then((e) => {
      console.log(e);
    })
    .catch((e) => {
  ...
  updateChunkSize(setting[chunkSizeKey])
    .then((e) => {
      console.log(e);
    })
    .catch((e) => {
